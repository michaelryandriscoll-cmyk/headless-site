...
function DropZone({ onFiles }) {
  ...
  return (
    <div onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={handleDrop} onClick={() => inputRef.current?.click()}
      ...
      <div style={{ fontSize: "13px", color: "#6b7280" }}>PDF, JPG, PNG up to 25MB each · Bank statements &amp; signed application</div>
    </div>
  );
}
...
export default function ThankYouPage() {
  ...
  useEffect(() => {
    try {
      const stored = sessionStorage.getItem("sbcLeadInfo");
    ...
    const init = async () => {
      try {
        const res = await fetch("/api/upload/init", {
          ...
          body: JSON.stringify({ name: leadInfo.name || "", email: leadInfo.email || "", phone: leadInfo.phone || "", loanAmount: leadInfo.loanAmount || "", businessName: leadInfo.businessName || "" }),
        });
  ...
  const uploadFile = async (docId, file, fileIndex) => {
    ...
    try {
      const presignRes = await fetch("/api/upload/presign", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId, fileName: file.name, fileType: file.type, fileSize: file.size, fileIndex }) });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const completeRes = await fetch("/api/upload/complete", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId }) });
  ...
  return (
    <>
      <Script id="google-ads-conversion" strategy="afterInteractive">{`gtag('event', 'conversion', {'send_to': 'AW-18096733041/L5peCKT6qZ0cEPH2mLVD', 'value': 1.0, 'currency': 'USD'});`}</Script>
        ...
        <div style={{ maxWidth: "680px", margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: "32px" }}>
            <a href="/" style={{ textDecoration: "none" }}>
              <div style={{ color: "#ffffff", fontSize: "22px", fontWeight: "800", letterSpacing: "-0.5px", marginBottom: "24px" }}>Small Business Capital</div>
            </a>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", marginBottom: "32px" }}>
              {["Application", "Your Info", "Upload Docs"].map((label, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center" }}>
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                    <div style={{ width: "32px", height: "32px", borderRadius: "50%", background: i < 2 ? "#00a870" : "#2563eb", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "14px", fontWeight: "700", color: "#fff" }}>{i < 2 ? "✓" : "3"}</div>
                    <div style={{ fontSize: "11px", color: i < 2 ? "#00a870" : "#93c5fd", marginTop: "4px", whiteSpace: "nowrap" }}>{label}</div>
                  </div>
                  {i < 2 && <div style={{ width: "60px", height: "2px", background: "#00a870", margin: "0 4px", marginBottom: "18px" }} />}
                </div>
              ))}
            </div>
            <div style={{ fontSize: "13px", color: "#7fb3d3", fontWeight: "500", letterSpacing: "1px", textTransform: "uppercase", marginBottom: "8px" }}>Step 3 of 3</div>
            <h1 style={{ color: "#ffffff", fontSize: "28px", fontWeight: "800", margin: "0 0 12px", lineHeight: "1.2" }}>{done ? "Documents Received ✓" : "Upload Your Documents"}</h1>
            <p style={{ color: "#93c5fd", fontSize: "16px", margin: 0, lineHeight: "1.5" }}>{done ? "We'll review your documents and be in touch within 1 business day." : "Upload your bank statements and signed application to complete your funding request."}</p>
          </div>
          ...
          <div style={{ background: "#ffffff", borderRadius: "16px", padding: "32px", boxShadow: "0 20px 60px rgba(0,0,0,0.3)" }}>
            {done ? (
              ...
              <div style={{ textAlign: "center", padding: "20px 0" }}>
                ...
                <h2 style={{ color: "#0f2342", fontSize: "20px", fontWeight: "700", marginBottom: "12px" }}>Upload portal temporarily unavailable</h2>
                <p style={{ color: "#4b5563", lineHeight: "1.6" }}>Please email your documents directly to <a href="mailto:michael@smallbusiness.capital" style={{ color: "#2563eb", fontWeight: "600" }}>michael@smallbusiness.capital</a> — we'll process them the same way.</p>
              </div>
            ) : (
              <>
                ...
                <div style={{ background: "#fffbeb", border: "1px solid #fde68a", borderRadius: "8px", padding: "12px 16px", marginBottom: "20px", fontSize: "13px", color: "#92400e" }}>
                  <strong>Don't have everything ready?</strong> That's okay — upload what you have now and email the rest to <a href="mailto:michael@smallbusiness.capital" style={{ color: "#92400e" }}>michael@smallbusiness.capital</a>. Reference your name and business name.
                </div>
                ...
                <button onClick={handleSubmit} disabled={anyUploading || !sessionId}
                  style={{ width: "100%", background: anyUploading || !sessionId ? "#9ca3af" : requiredFilled ? "#00a870" : "#2563eb", color: "#ffffff", border: "none", borderRadius: "10px", padding: "16px", fontSize: "16px", fontWeight: "700", cursor: anyUploading || !sessionId ? "not-allowed" : "pointer", transition: "background 0.2s ease" }}>
                  {submitting ? "Uploading your documents…" : !sessionId ? "Preparing secure portal…" : "Submit Documents Securely →"}
                </button>
          ...
          {!done && (
            <div style={{ textAlign: "center", marginTop: "24px", color: "#7fb3d3", fontSize: "13px" }}>
              Your application is already submitted ✓ — documents can be uploaded anytime.<br />
              Questions? <a href="tel:18889008979" style={{ color: "#93c5fd" }}>(888) 900-8979</a>
            </div>
          )}
        </div>
      </main>
